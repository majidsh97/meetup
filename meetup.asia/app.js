...
function leave(sid) {
    return new Promise((resolve, rejects) => {
        sql_1.conn.query('select sid,room from socket where sid=? or room=?;', [sid, sid], (err, res) => {
            if (err)
                throw err;
            if (res.length > 0) {
                if (res[0].sid === sid) {
                    sql_1.conn.query('delete from socket where sid=?;', [sid], (err) => {
                        if (err)
                            throw err;
                        if (res[0].room != null) {
                            join(res[0].room);
                    ...
                    sql_1.conn.query('update socket set room=NULL where room=?;', [sid], (err) => { if (err)
...
function join(sid) {
    sql_1.conn.query('select sid,room from socket where room is null order by RAND() limit 1;', [], (err, res) => {
        ...
        var_dump(res);
        if (res.length == 0) {
            sql_1.conn.query("insert into socket (sid) values (?) ;", [sid], (err, res) => {
                if (err)
                    throw err;
            ...
            sql_1.conn.query('update socket set room=? where sid=? ;', [sid, res[0].sid], err => {
                if (err)
                    throw err;
...
function towho(sid) {
    return new Promise((resolve, reject) => {
        sql_1.conn.query('select sid,room from socket where sid=? or room=? limit 1;', [sid, sid], (err, res) => {
            if (err)
                throw err;
            if (res.length > 0) {
                let t;
...
io.on('connect', (socket) => {
    ...
    socket.on('disconnecting', () => {
        leave(socket.id).then((s) => { if (s != null)
